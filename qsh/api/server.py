...
from .routes import status, config, sysid, health, wizard, rooms, backup, schedule, away, control, history, entities, trends, historian, balancing, swarm, source_selection, comfort_schedule, scop, snapshots, forecast, manual, mv, predictive
...
def create_app() -> FastAPI:
    ...
    # INSTRUCTION-478A — predictive-occupancy learner report.
    app.include_router(predictive.router, prefix="/api", tags=["predictive"])
    # INSTRUCTION-403 — M&V savings ledger (status + summary, per-day report).
    app.include_router(mv.router, prefix="/api", tags=["mv"])
    ...
    # INSTRUCTION-225C — operator MANUAL/AUTO override for direct-TRV positions.
    app.include_router(manual.router, prefix="/api", tags=["manual"])
    # INSTRUCTION-208A — DFAN forecast extension routes (router defines its own /api/forecast prefix)
